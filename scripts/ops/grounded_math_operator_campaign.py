...
def _prompt_candidates(document: dict) -> tuple[list[dict], list[dict]]:
    ...
    sum_templates = ("Tính tổng số lượng toàn bộ các dòng trong BOM {doc}.",)
    for template_id, template in enumerate(sum_templates, start=1):
        candidates.append(
            {
                "operation": "sum",
                "template_id": f"sum-{template_id}",
                "operand_labels": [],
                "operand_style": "document_aggregate",
                "prompt": template.format(doc=file_name),
            }
        )
    ...
    for left, right, operand_style in multiply_pairs[:MAX_CARDS_PER_DOCUMENT_OPERATION]:
        candidates.append(
            {
                "operation": "multiply",

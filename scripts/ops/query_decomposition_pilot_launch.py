...
def prepare_consolidated_launch(
    *, source_root: str | Path, source_commit: str,
    ...
    pilot_run_root = (target.parent / "run").resolve()
    if os.path.lexists(pilot_run_root):
        raise ValueError("pilot_run_root_must_be_never_used")
    pilot_run_root_relative = pilot_run_root.relative_to(root).as_posix()
    ...
    draft = {
        ...
        "pilot_run_root": pilot_run_root_relative,
...
def finalize_consolidated_launch(
    *, draft_path: str | Path, approval_path: str | Path,
    ...
    pilot_run_root_value = str(draft.get("pilot_run_root") or "")
    pilot_run_root = (root / pilot_run_root_value).resolve()
    pilot_run_root_valid = all((
        pilot_run_root_value == Path(pilot_run_root_value).as_posix(),
        pilot_run_root.is_relative_to(root / ".local"),

...
def _manifest_valid(manifest: dict) -> bool:
    ...
    if any(set(card) != required for card in cards):
        return False
...
def _default_still_off(release_decisions: dict) -> bool:
    grounded_math = release_decisions.get("decisions", {}).get(
        "RAG_GROUNDED_MATH_ENABLED", {}
    )
    return release_decisions.get("status") == "incomplete" and grounded_math.get(
        "decision"
    ) is None
...
def evaluate_operator_gate(
    ...
    effective_release_decisions = current_release_decisions or start_release_decisions

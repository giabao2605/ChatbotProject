    ...
    statements = [
        ...
        USING (
            SELECT fn.NodeID SourceNodeID, dn.NodeID TargetNodeID, 'HAS_VERSION' RelationType,
                   ...
                   COALESCE(page_ref.SourceQuote, bom_ref.SourceQuote) SourceQuote
            FROM dbo.TaiLieu t
            ...
            OUTER APPLY (
                SELECT TOP 1 PageNo, LEFT(COALESCE(
                    NULLIF(LTRIM(RTRIM(TextExtract)),''), NULLIF(LTRIM(RTRIM(LocalOCRText)),''),
                    NULLIF(LTRIM(RTRIM(VisionSummary)),'')
                ), 2000) SourceQuote
                FROM dbo.DocumentPages WHERE DocID=t.DocID
                  AND COALESCE(NULLIF(LTRIM(RTRIM(TextExtract)),''), NULLIF(LTRIM(RTRIM(LocalOCRText)),''),
                               NULLIF(LTRIM(RTRIM(VisionSummary)),'') ) IS NOT NULL
                ORDER BY PageNo
            ) page_ref
            OUTER APPLY (
                SELECT TOP 1 TrangSo PageNo, LEFT(NULLIF(LTRIM(RTRIM(RawRowJson)),''), 2000) SourceQuote
                FROM dbo.BangKeVatTu WHERE DocID=t.DocID
                  AND NULLIF(LTRIM(RTRIM(RawRowJson)),'') IS NOT NULL
                ORDER BY TrangSo, ID
            ) bom_ref
        ...
        WHEN NOT MATCHED THEN INSERT
             ...
             VALUES(source.SourceNodeID,source.TargetNodeID,source.RelationType,'deterministic','approved',1,
                    ...
                    source.SecurityLevel,source.SourceQuote,'deterministic-seed',SYSUTCDATETIME());
        """,
        """
        MERGE dbo.KnowledgeGraphEdge AS target
        USING (
        ...
        USING (
            SELECT dn.NodeID SourceNodeID, pn.NodeID TargetNodeID, 'HAS_PAGE' RelationType,
                   ...
                   t.ThuMuc Department, t.Site, ISNULL(t.SecurityLevel,'confidential') SecurityLevel,
                   LEFT(COALESCE(
                       NULLIF(LTRIM(RTRIM(p.TextExtract)),''), NULLIF(LTRIM(RTRIM(p.LocalOCRText)),''),
                       NULLIF(LTRIM(RTRIM(p.VisionSummary)),'')
                   ), 2000) SourceQuote
            FROM dbo.DocumentPages p JOIN dbo.TaiLieu t ON t.DocID=p.DocID
        ...
        USING (
            SELECT dn.NodeID SourceNodeID, pn.NodeID TargetNodeID, 'CONTAINS_PART' RelationType,
                   ...
                   t.ThuMuc Department, t.Site, ISNULL(t.SecurityLevel,'confidential') SecurityLevel,
                   LEFT(NULLIF(LTRIM(RTRIM(b.RawRowJson)),''), 2000) SourceQuote
            FROM dbo.BangKeVatTu b JOIN dbo.TaiLieu t ON t.DocID=b.DocID

...
| Grounded math                    | Có                                           | Có, gồm integration live | 3/3 pair đạt; window-06 và burst đều tombstone | Inconclusive | Contract prospective `grounded-math-3d-100-v1` đã được owner authorize; còn thiếu fresh pilot 72 giờ/100 request pass, owner review và interaction matrix; flag giữ tắt |
...
- Các đợt ProxyLLM `503 no_capacity` và Voyage `429` trước đây được giữ làm evidence lịch sử; fresh smoke bắt buộc phải pass trước mỗi window mới và provider failure trong campaign vẫn dừng fail-closed.
- Grounded Math được đánh giá độc lập với CRAG; owner đã authorize prospective contract `grounded-math-3d-100-v1` cho Math-only controlled-demo.
- Chưa có production pilot pass; window 3 ngày/100 mới phải hoàn tất đủ canonical gate, operator gate và human review trước mọi quyết định tiếp theo.
...
| 2.4 Grounded Math | `inconclusive`; staging đạt, contract `grounded-math-3d-100-v1` đã được authorize nhưng chưa có fresh pilot 72 giờ/100 request pass; owner review và interaction matrix chưa được mở |
...
| 2.4 Grounded Math         | Fixture live, 3/3 pair, exact Decimal/provenance/citation gate và rollback đều đạt | Window-06 và burst đã tombstone; fresh pilot `grounded-math-3d-100-v1`, owner review và interaction matrix chưa hoàn tất | `inconclusive`, flag tắt |
...
7. [Đã đạt] Series guardrail đạt mọi điều kiện nội tại của Grounded Math; capability này được đánh giá độc lập với CRAG.
...
9. [Đã authorize prospective 2026-08-14] Contract `grounded-math-3d-100-v1` mở fresh pilot đúng 100 eligible request trong tối thiểu 72 giờ, cap `35/24 giờ`, concurrency 1 và không retry/replacement/catch-up. Disposition cũ vẫn khóa lịch sử; owner review, interaction matrix và default rollout tiếp tục bị chặn cho tới khi pilot mới pass.
...
- Human-review checkpoint đã có validator/finalizer fail-closed. Mỗi review pack giữ `review_contract_sha256` để phát hiện thay đổi ngoài trường `human_review`; tracked anchor [`../data/integrated_hardening_v1/evidence/post-ingest-human-review-anchor-20260716.json`](../data/integrated_hardening_v1/evidence/post-ingest-human-review-anchor-20260716.json) khóa semantic hash của pack và raw SHA-256 của Graph queue lịch sử. Graph validator yêu cầu tối thiểu 20 edge và precision ít nhất 95%. Báo cáo chỉ giữ metadata và hash nguồn, không giữ raw question/answer/document, reviewer name hoặc note.
- Dry run tại `reports/controlled-demo/20260716-post-ingest-v3/review/finalization-anchored-pending.json` xác nhận anchor và contract của CRAG/Grounded Math khớp, đồng thời 21 Graph edge không bị đổi provenance; toàn bộ nhãn vẫn còn trống. Kết quả đúng là `all_review_inputs_complete=false`, `graph_review_ready=false`, `community_generation_unlocked=false`; đây là checkpoint chờ người review, không phải lỗi hệ thống.
- Sau khi điền review, chạy `scripts.controlled_demo_eval.finalize_reviews` theo lệnh trong [`controlled-demo-post-ingest-test-plan.md`](controlled-demo-post-ingest-test-plan.md). Validator không tự chấp nhận milestone và không tự mở Community Summaries; nếu Graph review hợp lệ, bước kế tiếp vẫn là chạy lại Graph quality gate rồi mới ghi decision artifact.
...
- [ ] Grounded Math có staging baseline/candidate đạt, nhưng window-06 và burst đã tombstone; contract `grounded-math-3d-100-v1` yêu cầu fresh pilot tối thiểu 72 giờ/100 request hợp lệ trước owner review, interaction matrix và decision cuối.
...
- Commit `7d8cfa2` thay correction hybrid thứ hai bằng Qdrant payload lookup giới hạn khi có governed `base_code`, vẫn giữ strict RBAC/site/security/lifecycle filter và hybrid fallback một lần. Diagnostic alias 5/5 đạt, retry/error 0, correction P95 `249 ms`, giảm `88.2%` so với `2110 ms`.
- Window `20260729-crag-window-03-7d8cfa2` được predeclare sau CI run `30422015073`, với arm order baseline-first/candidate-first/baseline-first. Pair 01 fail quality gate: candidate 5/9, bốn ProxyLLM generation error và hai Voyage rerank error; latency ratio `0.434`, cost ratio `0.675`, leakage 0 và request budgets vẫn đạt. Pair 02/03 không chạy; không tạo series, authorization hoặc bật flag.

...
Đây là tài liệu chuẩn bị offline cho một campaign Grounded Math controlled-demo
hợp lệ trong tương lai. Nó không phải review evidence, owner decision,
activation bundle hoặc quyền chạy traffic hay bật tính năng.

Burst `20dfd03b262a2eedf15731d7` đã hoàn tất 100 request nhưng canonical gate bị
`rejected` do `owner_declaration=false`. Owner disposition hiện tại là
`deferred_pending_valid_pilot`; vì vậy review set chưa được khóa, owner review
chưa bắt đầu và interaction matrix chưa được chạy.
...
- exact serving commit, runtime identity, activation scope, corpus fingerprint,
  manifest hash, owner declaration và operator-tool hash khớp current window;
...
4. Với mỗi case, định nghĩa `stratum` chính xác là tuple `(operation,
   operand_style, eligible_outcome, document_identity_sha256)`.
5. Điền từng slot bằng candidate thuộc stratum đang có ít selected case nhất.
   ...
   với `trace_id_sha256`, rồi lấy case đầu chưa chọn.
...
Quy tắc tie-break trên phải được bind rõ trước khi sử dụng; tài liệu này không
tự sửa campaign declaration đã ký.
...
| `formula` | Công thức khớp operation và không thêm phép biến đổi ngoài phạm vi. |
...
| `provenance` | Source và document identity đúng với nguồn đang serve và được authorize. |
...
| `reason_code` | Controlled reason code giải thích judgement mà không chép raw content. |
...
tất. Sau khi cả hai gate pass và final RC đã freeze, matrix nhỏ nhất so sánh
`all_off` với Math-only ở concurrency 1 và 5. Query, CRAG, Claim Repair,
Community Summaries và Late Interaction nằm ngoài Math gate này, trừ khi chúng
đạt accepted độc lập.

Graph đang có disposition `keep_off_technical_limit`: tiếp tục OFF, không chạy
lại cùng thiết kế và không thêm vào matrix. Chỉ được mở lại sau khi owner duyệt
thay đổi provider, data-plane, corpus hoặc product scope và có declaration mới.

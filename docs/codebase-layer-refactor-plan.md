...
Trạng thái: **Completed — Phase 0 đến Phase 6 đã hoàn tất và được kiểm chứng**
...
| Golden ban đầu | Baseline và candidate cùng **1 passed, 2 failed** do database/Qdrant tại thời điểm benchmark chưa có hai mã `9.3.03951` và `9.3.03844`; cả hai arm trả cùng not-found fail-closed. Quality delta không regression nhưng gate ban đầu được ghi `inconclusive_data_missing`. Evidence lịch sử: `candidate/golden-eval.txt`. |
| Golden hậu corpus | Người dùng cung cấp 14 PDF gốc; từng file được render/kiểm tra, hash SHA-256, rồi đi qua `DocumentUpload -> IngestionRunner -> PublicationCoordinator` trên SQL/Qdrant chính theo phê duyệt. JobID/DocID `56-69` đều quality 100, published, servable, vectorized và sau review đều là `ver03`. Golden cũ được xác nhận sai nguồn: `Model1` của `9.3.03951` là `Inox 201`; bản `9.3.03844(...)-Model.pdf` có dung sai `±0.2 mm` và `±0.5 mm`. Contract mới buộc đúng filename đầy đủ và trang 1, không chấp nhận sibling PDF. Baseline `316b8fc` và candidate đều **5 passed**, SQL/Qdrant **2 passed**; blocker `inconclusive_data_missing` đã đóng. Evidence: `reports/refactor/phase-4/post-corpus/manifest.json`, fixture commit `b5145d2`. |
...
| Compatibility seam | `_ContextBoundEngine` là proxy không trạng thái để repository legacy tiếp tục dùng interface cũ. Nó không sở hữu connection và không phải process singleton; app/request hoặc CLI composition phải bind `RepositoryRuntime` trước khi dùng. Phase 5 dự kiến Phase 6 đánh giá việc xóa; inventory Phase 6 sau đó phân loại đây là public/unknown compatibility nên chỉ chuyển internal caller và giữ proxy cho đến external deprecation decision riêng. |
...
| Known issues | 19 integration/eval test là opt-in skip, không phải pass. Còn 1 warning `httpx`/`TestClient`. Extra owned-coverage run lỗi native reload không ảnh hưởng full artifact nhưng là hạn chế của cách chạy coverage trong một process. Tại thời điểm đóng Phase 5, `_ContextBoundEngine` là compatibility debt được giao Phase 6 đánh giá; Phase 6 sau đó xác nhận public/unknown status và giữ nó đến external deprecation decision. Ngoại lệ benchmark Phase 4 và bảy stage comparison đỏ vẫn còn nguyên; Phase 5 không biến chúng thành pass hoặc phê duyệt live rollout. |
...
mọi quyết định rollout vẫn phải dùng artifact/decision gate hiện hành.

### 9.7. Phase 6 — Xóa migration facade và compatibility debt nội bộ

Trạng thái: **Completed / Validated**. Phase 6 loại broad facade khỏi toàn bộ
caller nội bộ, nhưng giữ các surface public/unknown theo đúng chính sách
deprecation. Không chủ ý thay đổi HTTP/OpenAPI/SSE, schema dữ liệu, thuật toán
RAG, feature flag, collection hoặc rollout decision.

| Trường evidence | Kết quả thực tế |
|---|---|
| Baseline / candidate | Baseline `47e1e73` (`docs: close phase five composition gate`), candidate code `29c8caf` (`refactor: retire internal compatibility facade usage`), branch `codex/codebase-layer-refactor`. Artifact chính: `reports/refactor/phase-6/candidate/manifest.json`; summary: `candidate/validation-summary.json`. |
| Inventory và phân loại | `services/__init__.py`, `db/repository.py`, `rag/service.py`, `ingestion/pdf_processor.py` đều có external usage chưa biết nên được phân loại **public/unknown compatibility**. Không facade nào trong bốn file được phép xóa ở phase này. `db.repositories/__init__.py` là internal package surface nên bỏ wildcard và chỉ export owner modules. Compatibility aliases ở `api/app_server.py` cũng giữ vì public status chưa rõ. |
| Surface được giữ | Static `services` facade giữ đúng **137** tên, hash `3bf4c3445045f05bec10fe268ae8a1c168ece4e99acfe8a1a86739d2bfbba25d`. `db.repository` giữ **206** tên, hash `6246a17b7d6f7b3bed3bcb636e1e624d97ad002fd4b5004f54f402e60225b442`. `rag.service` giữ **84** tên, hash `b9c0c19d3346ccd6bca0f3b56f2e236401373f91fd1d8736fc985a2239cd0a57`. `pdf_processor` giữ **47** tên, hash `75e8a51e3adab687cb2e879efd0869ddf44bd0f42fbcc4f1a675e6837989e87a`. Test còn khóa identity với owner thật và pure behavior đại diện; `chat_with_rag is rag.pipeline.chat_with_rag`. |
| Internal migration | Production/API/RAG/ingestion/worker/service và operational scripts import owner module trực tiếp. `services/__init__.py` không còn dynamic `_SERVICE_MODULES`/`globals()` discovery. `db.repositories` không còn wildcard. Ratchet table-driven cấm internal import qua bốn legacy facade trên cả `src` và `scripts`, kể cả cú pháp `from package import module`. Test-only layered compatibility import còn lại được ghi rõ và không phải production caller. |
| RAG process lifecycle | Review phát hiện RAG process sau Phase 5 chưa sở hữu SQL runtime. Candidate thêm `RagDatabaseRuntime` composition, build/bind/close SQL theo lifespan, bind request middleware và bind lại SQL/Qdrant ContextVar trong cả `/chat` và `/chat/stream` executor thread. Nếu model startup lỗi nhưng SQL còn tốt, history/feedback vẫn phục vụ; nếu SQL runtime thật sự thiếu, route DB-backed trả `503` thay vì `403` sai nghĩa. State luôn reset kể cả resource `close()` lỗi, và ContextVar được test reset sau worker. |
| Script lifecycle | `check_qdrant_schema.py` và `check_image_summary_coverage.py` tự tạo/đóng configured runtime, không hardcode collection. `alter_ingestionjobs.py` import-safe, tự sở hữu SQL runtime, kiểm `COL_LENGTH` để idempotent và không nuốt lỗi SQL. Các danger/backup/reconcile/eval scripts dùng owner module hoặc configured maintenance runtime; import smoke không chạy migration hay destructive operation. |
| Architecture baseline correction | Lần chạy full architecture đầu Phase 6 phát hiện Phase 5 chỉ ghi kết quả của `test_layering_contracts.py`, chưa chạy toàn bộ ratchet: allowlist cũ còn stale và `api/app_server.py` còn cạnh DB construction. Phase 6 sửa prerequisite bằng composition-owned app DB builder, thu gọn allowlist còn đúng **41** public/unknown wildcard/service findings và chạy lại full architecture **18 passed**. Đây là correction evidence, không được trình bày như baseline Phase 5 đã xanh toàn bộ. |
| Full backend / coverage | JUnit `candidate/pytest-full.xml`: **2.169 collected, 2.147 passed, 22 opt-in skipped, 0 failed/error**, 1 warning đã biết. `candidate/coverage-backend.json`: **92,217489% line / 84,791362% branch**, checker 80/80 pass. Từ cùng artifact authoritative, denominator **38** file production thay đổi đạt **90,452376% line (3.979/4.399)** và **81,25% branch (936/1.152)**. Secondary multi-`--cov` run lỗi native `onnxruntime/numpy` reload nên không được dùng làm evidence. |
| SQL/Qdrant chính | Theo phê duyệt người dùng, `RUN_DB_TESTS=1` và `RUN_QDRANT_TESTS=1` chạy read-only trên database/Qdrant chính với snapshot `phase3-ingestion-consistency-snapshot-v1`: **2 passed**. Không tạo fixture, ingest, re-ingest, sửa hoặc xóa dữ liệu. Evidence: `candidate/sql-qdrant-consistency.xml`. |
| HTTP/SSE/frontend | Candidate OpenAPI app **116 paths** và RAG **8 paths** bằng byte-for-byte với Phase 4 baseline. `sse-success.jsonl`, `sse-busy.jsonl` và upload/review samples có hash không đổi. Vue Vitest **11 files / 32 tests passed**; `vue-tsc` + Vite build pass. |
| Import / hygiene | `compileall` pass. Import smoke pass cho **4** process modules và **8** changed operational scripts, không tạo connection/model hoặc chạy destructive main. `git diff --check` pass; stale internal-facade grep sạch ngoài docstring của chính facade và test-layered compatibility được giữ chủ đích. Hai folder nguồn `9.3.03844` và `9.3.03951` không bị stage, sửa hoặc xóa. |
| Test lifecycle | Giữ architecture ratchet, compatibility identity/behavior, RAG resource/context lifecycle và endpoint contracts vì đây là hàng rào lâu dài. Các test patch facade cũ được chuyển sang owner seam. Ba test environment mutation cũ từ Phase 5 được chuyển sang explicit runtime values. Không giữ test đo lường tạm và không xóa test nào chưa có replacement cùng semantics. |
| Review / security | Hai review độc lập Standards và Spec đều kết thúc không còn finding CRITICAL/HIGH/MEDIUM/LOW. Scan staged diff không phát hiện credential production; các match chỉ là dummy test settings. `pip-audit` không có trong `chat_env`, vì vậy dependency vulnerability audit chưa được xác nhận và không được trình bày như pass. |
| Compatibility debt còn lại | `_ContextBoundEngine`, bốn public/unknown facade và app compatibility aliases chỉ được xóa sau external caller inventory + deprecation decision riêng. 22 integration/eval skip là opt-in, không phải pass. Còn `StarletteDeprecationWarning`. Ngoại lệ benchmark Phase 4 và bảy stage comparison đỏ giữ nguyên; Phase 6 không biến chúng thành performance pass hoặc live-release approval. |
| Rollback | Revert `29c8caf` để hoàn tác toàn bộ code Phase 6 về closure Phase 5 `47e1e73`; commit docs/artifact closure được revert riêng. Không cần schema/data/Qdrant rollback vì acceptance chỉ đọc dữ liệu và migration script không được chạy. |

Phase 6 hoàn tất mục tiêu nội bộ: import path production thể hiện owner thật,
facade public chưa rõ caller vẫn tương thích và được ratchet riêng. Roadmap
refactor theo tầng trong tài liệu này đã hoàn tất, nhưng không tự động bật
feature hoặc phê duyệt production/live rollout.

...
## Checkpoint Grounded Math default rollout và Query readiness — 2026-08-21
...
## Checkpoint thực thi — 2026-08-14 (lịch sử, superseded bởi checkpoint 2026-08-21)
...
- Phase 3 — Query-only manifest `10+3`, terminal no-render contract, trace binding, serialized branch retrieval, explicit external-processing policy, provider-boundary revalidation, deployment/restore readiness, atomic formal dispatch và preparation-packet binding đã được harden đến clean commit `d424628`. Formal window `d424628` có pair 01 pass nhưng pair 02 fail duy nhất latency P95 ratio `2.538923543 > 1.5`; zero provider failure/retry, mọi non-latency gate pass và pair 03 không chạy. Offline exact-gate replay và minimized counterfactual cho thấy ideal branch parallelism riêng lẻ vẫn RED `1.966006443 > 1.5`; không có minimal code-local fix/test seam chứng minh đồng thời giữ shared-client safety, parent evidence semantics và threshold khóa. Authorization/window đã consumed/tombstone; Query có disposition `keep_off_technical_limit_current_design` và vẫn OFF. Chỉ một future architecture scope được duyệt riêng cho batched/isolated retrieval + parent context mới có thể mở clean commit và formal series hoàn toàn mới; không same-window rerun, threshold relaxation hoặc carry-forward.

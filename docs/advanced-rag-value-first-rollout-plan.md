...
- Phase 5 — đã bắt đầu trên `0b98e12`: preflight `9/9`, smoke `5/5`, interleaved diagnostic chạy đủ hai pair nhưng `inconclusive` do order-sensitive latency/provider variance. Chưa có root-fix signal hợp lệ, chưa mở formal window; lượt kế tiếp phải dùng declaration và smoke mới trong capacity window khác.

...
- Mọi Query window trước clean RC hiện tại chỉ còn là lịch sử. Không reuse smoke, trace, preflight, rollback, elapsed time hoặc pair từ bất kỳ window cũ nào; không chạy thêm standalone health smoke. Gate tiếp theo là owner authorization mới cho exact clean commit và fresh run-root, provider-boundary revalidation rồi đúng một fresh formal-window smoke `5/5`, zero retry. Nếu smoke pass mới tạo declaration/trace và chạy tối đa ba formal pair tuần tự, stop-on-first-failure.
...
- Phase 3 — Query-only manifest `10+3`, terminal no-render contract, trace binding, strict deterministic local split, provider-boundary revalidation và deployment/restore readiness đã được harden đến exact commit `d28be06`. Recovery health proof mới đạt `5/5` nhưng đã consumed và không formal-eligible; quality vẫn chưa được đánh giá trong series mới, formal pair `0`, Query vẫn OFF. Gate tiếp theo cần authorization/declaration/run-root/smoke/trace hoàn toàn mới trên một exact clean commit và không carry-forward.

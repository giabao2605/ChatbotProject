...
- Theo authorization tạo target nhưng chưa bật feature, restore drill đã tạo SQL database `Mech_Chatbot_DB_RestoreTest_RAGPilot_20260810_4bc666c` và Qdrant collection `TaiLieuKyThuat_v2_RestoreTest_RAGPilot_20260810_4bc666c` với `231/231` point; verifier exact `4bc666c` pass trên restore evidence SHA-256 `1f69e8bcdd87147f9d526bd0b3cf0738ad16c9576d4d10e2396b6258d5d7af27`. Owner `bao.nguyen` đã chấp nhận exact snapshot fingerprint `b1a73fa7d90500e5c738154e30b64ade68a0b59e5de7066f0e7981e613209b11`; Graph controlled-demo decision SHA-256 `07e5ae3eb6257844df521519efa99410adbe629129a02be00ab59eabef324566`, decision ledger SHA-256 `87bacc533d460d78ea8cc649d1649b1a3514b3fcdad1a831cfb0d0848c798a1b`, activation bundle SHA-256 `ce06682674819d3ae82b457ff096d6b1bfb73eda9b13b7046ef31f29c4563234`. Quyền này chỉ áp dụng cho Graph `controlled_demo`; default rollout vẫn OFF.
...
- Owner-decision packet `.local/advanced-rag-owner-decision-packet-20260810.json` ghi `approved_recommended_all_three`: Query manifest adjudication, Graph named disposable target creation, và Math tombstone + clean restart. Sau khi owner chấp nhận exact Graph fingerprint, packet được mở rộng để bind Graph controlled-demo decision/ledger/bundle; mọi default rollout enablement và git push vẫn bị loại trừ.
...
- Phase 3 — owner đã adjudicate current-contract drift. Query-only manifest mới giữ floor `10+3`, không phụ thuộc Math và khóa terminal no-render contract; ba nhãn BOM Math-coupled được tách nguyên vẹn sang interaction manifest. Generator/preflight/regression suite và served-evidence root fix đã GREEN. Formal pair mới nhất vẫn là tombstone RED do provider retry/post-check refusal; capacity recovery smoke cũng chưa sạch, nên chỉ được mở một series mới sau fresh `5/5`, `0` retry smoke. Query vẫn OFF.
- Phase 4 — formal Graph gate đã hoàn tất trên `4bc666c`: queue current-contract `21/21` được hai reviewer duyệt; preflight, rollback, ba provider smoke, ba graph-only matched pair và recomputed series guardrail đều pass. Disposable SQL/Qdrant targets đã được restore và verifier pass với exact fingerprint `b1a73fa7d90500e5c738154e30b64ade68a0b59e5de7066f0e7981e613209b11`. Owner đã chấp nhận fingerprint và Graph controlled-demo decision/bundle đã được tạo hash-bound vào exact series/ledger/fingerprint; Graph đạt `pilot-authorized` cho `controlled_demo` nhưng chưa `live-authorized` cho default rollout. Bước gated kế tiếp là start/verify runtime Graph-only trên detached checkout sạch và mở LAN pilot đủ 7 ngày/100 routed relational request.

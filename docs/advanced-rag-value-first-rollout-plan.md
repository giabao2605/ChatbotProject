...
- Vì telemetry/runtime contract đã thay đổi sau `4bc666c`, mọi Graph decision/bundle/runtime/formal artifact cũ chỉ còn là lịch sử và không được authorize source mới. Chưa có Graph runtime hoặc pilot window đang active. Bước gated kế tiếp là freeze detached RC sạch từ HEAD chứa checkpoint này, bind lại governance/source commit, verify restore/preflight, chạy provider smoke `5/5` và mở đúng một formal series mới gồm ba matched pair; stop-on-first-failure và tombstone, không bật default rollout.
...
- Phase 4 — historical technical gate trên `4bc666c` đã pass nhưng runtime `8103/8104` đã dừng và artifact cũ không carry-forward qua telemetry/runtime change. Owner đã chấp nhận phương án Graph-specific `single_owner` cho pilot `20` case; formal Graph vẫn khóa `multi_reviewer/independent`. Implementation telemetry/gate fail-closed đã land tại `e165e5c` và qua regression/coverage/security review. Graph hiện `OFF`, chưa có active pilot window và chưa còn `pilot-authorized` trên source mới. Bước kế tiếp: clean detached RC từ HEAD checkpoint, governance rebind, restore/preflight, smoke `5/5`, ba formal pair + recomputed series; chỉ khi toàn bộ pass mới tạo controlled-demo decision/bundle/runtime và mở window 7 ngày/100 routed request.

...
- Phase 4 — formal attempt trên `5732c42` và reversed-arm supporting diagnostic trên `12caa36` đều đã đóng fail-closed. Diagnostic xác nhận p95/gate đổi mạnh theo arm order (`2.189261` RED so với `0.884741` GREEN) và pair-01 candidate có Qdrant transport fallback; exact `3d541ab` đã harden trace-error detection nhưng không được dùng để sửa ngược evidence. Không có formal series/decision/bundle/runtime/pilot mới; Graph vẫn OFF. Bước gated kế tiếp cần owner adjudicate một materially different latency design delta trước declaration mới, không được rerun cùng design, chọn kết quả đẹp hoặc nới oracle. Governance pilot `single_owner` 20 case đã được chấp nhận nhưng chỉ được dùng sau khi một formal series mới đạt trọn vẹn; formal vẫn bắt buộc `multi_reviewer/independent`.

...
- Mọi checkpoint Query trước `58dbb08` chỉ còn là lịch sử. Không reuse smoke, trace, preflight, rollback, elapsed time hoặc pair từ bất kỳ window cũ nào. Không mở thêm smoke theo lịch/manual retry; gate tiếp theo chỉ được mở sau tín hiệu recovery bên ngoài có timestamp và provider identity, bằng owner authorization mới, run-root hoàn toàn mới và đúng một fresh smoke `5/5`, zero retry. Nếu smoke pass mới chạy tối đa ba formal pair tuần tự, stop-on-first-failure.
...
- Phase 3 — Query-only manifest `10+3`, terminal no-render contract, trace binding, strict deterministic local split và provider-boundary revalidation đã được harden đến exact commit `58dbb08`. Latest offline/pre-boundary checks đạt, nhưng fresh smoke fail `0/5` với năm HTTP `502` trước formal execution; quality chưa được đánh giá, formal pair `0`, Query vẫn OFF. Cả hai window `a88c0bc` và smoke-only root `58dbb08` đều là tombstone; gate tương lai cần recovery signal bên ngoài, authorization/declaration/run-root/smoke/trace hoàn toàn mới và không carry-forward.

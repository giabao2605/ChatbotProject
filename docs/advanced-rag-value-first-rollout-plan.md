...
- Graph control/candidate runtime đã start trên detached checkout sạch `4bc666c` tại `8103/8104`. Health có xác thực xác nhận control `all_off`, candidate chỉ bật `RAG_GRAPH_RETRIEVAL_ENABLED`, cùng exact SQL/Qdrant target, fingerprint, bundle và restore evidence SHA; runtime identity lần lượt `0a47442886d5d2dbba8b5ea5cd2c48dbd414a3518937acd130dea35c51ab9526` và `ff4596e224804fbae3bc17dc938b3069fda92a822339dcd56dbb57108973fefc`. Runtime receipt `.local/graph-cycle-4bc666c/graph-controlled-demo-runtime-receipt.json` SHA-256 `82e2a1967d1f12b65e0f283100305fb0d2b5e70a4d039af9324b6aec1a3dce18` pass toàn bộ check. Chưa mở/count pilot window: Graph còn thiếu governance artifact đúng `commit/scope` ký trước pilot cho human review `20` case; không kế thừa Math `single_owner` exception.
...
- Phase 4 — formal Graph gate đã hoàn tất trên `4bc666c`: queue current-contract `21/21` được hai reviewer duyệt; preflight, rollback, ba provider smoke, ba graph-only matched pair và recomputed series guardrail đều pass. Owner đã chấp nhận fingerprint; Graph controlled-demo decision/bundle và runtime/restore receipt đều được verify, control `all_off`, candidate Graph-only đang healthy trên `8103/8104`. Graph đạt `pilot-authorized` cho `controlled_demo` nhưng chưa `live-authorized` cho default rollout. Pilot window chưa bắt đầu/count vì còn thiếu Graph-specific governance artifact ký trước pilot cho human review `20` case; sau signoff mới được khai báo window 7 ngày/100 routed relational request.

...
- Phase 3 — Query-only manifest `10+3`, terminal no-render contract, trace binding, explicit external-processing policy, provider-boundary revalidation, deployment/restore readiness, atomic formal dispatch và preparation-packet binding đã được harden. Formal window `d424628` có pair 01 pass nhưng pair 02 fail duy nhất latency P95 ratio `2.538923543 > 1.5`; zero provider failure/retry, mọi non-latency gate pass và pair 03 không chạy. Sau disposition `keep_off_technical_limit_current_design`, owner đã mở architecture scope mới và batched branch retrieval + decomposed-only parent batch đã freeze tại `9e48ddc`, full unit `3015/3015`, không provider traffic. Query vẫn OFF vì chưa có measured formal evidence trên design mới. Gate kế tiếp là fresh authorization/root/preparation/boundary/smoke/declaration/trace/series; không same-window rerun, threshold relaxation hoặc carry-forward.

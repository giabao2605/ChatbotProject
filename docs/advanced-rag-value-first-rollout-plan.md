...
- Phase 4 — V2 trên exact `fe57f7d` đã chạy đủ `17/17`, arm-order consistent, cost đạt và không có health/provider/execution failure, nhưng latency p95 ratio `2.226645` vẫn RED; một tail đồng thời qua nhiều stage/provider ở case đầu chi phối max-like p95 trong khi Graph traversal chỉ `333` ms, còn cold-start chưa được chứng minh. Candidate `10/17`, baseline `9/17`; supporting diagnostic không chạy full quality gate. Không có formal series/decision/bundle/runtime/pilot mới; Graph vẫn OFF. Bước gated kế tiếp cần owner chọn symmetric unmeasured warm-up design mới hoặc feasibility review; cấm same-design rerun, bỏ case đầu, đổi percentile hoặc nới oracle. Governance pilot `single_owner` 20 case chỉ dùng sau khi formal series mới đạt trọn vẹn; formal vẫn bắt buộc `multi_reviewer/independent`.

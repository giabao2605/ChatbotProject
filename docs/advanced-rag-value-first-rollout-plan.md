...
- Phase 4 — owner đã adjudicate và V2 materially-different design đã được triển khai trên exact `9b2a398`: `17` adjacent per-case pair, alternating first arm, health probe trước từng pair, exact per-case fingerprint và stage latency riêng. Cửa sổ mới dừng fail-closed ở `4/17` khi candidate `graph-uses-material` phát một `bm25_retrieval` error+fallback; do đó chưa có latency/cost/arm-order conclusion và không được chạy tiếp hoặc carry-forward. Không có formal series/decision/bundle/runtime/pilot mới; Graph vẫn OFF. Bước gated kế tiếp là externally confirmed retrieval/provider recovery rồi declaration/window mới từ cặp đầu tiên. Governance pilot `single_owner` 20 case đã được chấp nhận nhưng chỉ dùng sau khi formal series mới đạt trọn vẹn; formal vẫn bắt buộc `multi_reviewer/independent`.

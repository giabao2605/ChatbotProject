...
- Phase 4 — final clean RC `5732c42` đã hoàn tất governance rebind, restore reconciliation, preflight, rollback và provider recovery smoke; formal giữ đúng `multi_reviewer/independent`. Hai pair hợp lệ đầu pass nhưng pair 03 fail duy nhất latency p95 ratio `2.419403 > 1.5`; provider tail chiếm `97.84%` delta và chưa có code-controlled root-fix signal. Attempt đã tombstone, không có series/decision/bundle/runtime/pilot được tạo; Graph vẫn OFF. Bước gated kế tiếp cần owner adjudicate một design delta cho window mới, không được rerun chọn kết quả đẹp hoặc nới oracle. Pilot `single_owner` 20 case chỉ được dùng sau khi một formal series mới đạt trọn vẹn.

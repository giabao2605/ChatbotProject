...
| Regression/targeted/full tests | Codex | Đã đạt trên commit `6c6e828`: local backend/frontend/security xanh; current-commit GitHub Actions `unit` và `frontend` đều `success` | Evidence CI: run `30416378397`; local artifact `tmp/coverage-mech-chatbot-fast.json` ghi line `90.71%`, branch `85.02%` |
| Self-review hai trục Standards/Spec | Codex + sub-agent | Review trong task hiện tại không có finding chặn, nhưng chưa có run-local review artifact nên không được tính là release evidence | Capture hoặc chạy lại review artifact trước bất kỳ gateway approval nào |
| Rollback CRAG + Claim Repair | Codex | Đã đạt trên runtime commit `6c6e828`: hai flag false và hai rollback test đạt | Evidence: `reports/controlled-demo/20260729-crag-6c6e828/rollback.json` |
| Provider smoke cho pair 01 | Codex chạy lệnh; provider phải sẵn sàng | Đã đạt 5/5, 0 retry, provider không blocked; process evaluation pin `MAX_CONCURRENT_RAG=4` mà không sửa `.env` | Evidence: `reports/controlled-demo/20260729-crag-6c6e828/pair-01/provider-smoke.json` |
| Ba staging baseline/candidate pairs | Codex chạy lệnh; SQL/Qdrant/provider phải sẵn sàng | Dừng fail-closed sau pair 01 trên commit `6c6e828`: candidate đạt 9/9, không retry, cost đạt nhưng P95 `12682/9473 = 1.339 > 1.25`. Outlier `crag-admin-exception` nằm ở generation provider (`11152 ms` so với baseline `5376 ms`), không có correction/repair; retrieval candidate không chậm hơn và Voyage cùng fallback không retry ở cả hai arm. Không chạy pair 02/03, không tạo series/authorization | Evidence: `reports/controlled-demo/20260729-crag-6c6e828/pair-01/run.json`, `gate.json`, `baseline/trace.json`, `candidate/trace.json` |

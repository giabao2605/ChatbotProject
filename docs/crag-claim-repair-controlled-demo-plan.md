...
| Regression/targeted/full tests | Codex | Runtime commit `7d8cfa2` đạt targeted suite, full backend line `92.23%`/branch `84.98%`, frontend test/build và GitHub Actions `unit`/`frontend` | Evidence CI của runtime commit: run `30422015073` |
...
| Rollback CRAG + Claim Repair | Codex | Đã đạt trên runtime commit `7d8cfa2`: hai flag false và hai rollback test đạt | Evidence: `reports/controlled-demo/20260729-crag-fastpath-validation-7d8cfa2/rollback-test.json` |
| Provider smoke cho từng pair | Codex chạy lệnh; provider phải sẵn sàng | Window 04 pair 01: lần đầu bị policy local `internal_only` chặn trước egress và được giữ lại; smoke evaluation process-only sau đó đạt 5/5, 0 retry. Pair 02/03 không chạy do latency gate fail | Evidence: `reports/controlled-demo/20260729-crag-window-04-9b6f3ec/provider-smoke-01.json`, `provider-smoke-01-authorized.json` |
| Ba staging baseline/candidate pairs | Codex chạy lệnh; SQL/Qdrant/provider phải sẵn sàng | Window `20260729-crag-window-04-9b6f3ec` predeclare order candidate-first/baseline-first/candidate-first. Pair 01: baseline 8/9, candidate 9/9, provider failure 0/0, wrong-refusal giảm 1→0, correction và repair đều exercise đúng một lần, cost ratio 0.9989. Gate chỉ fail latency ratio `1.5426 > 1.25`; pair 02/03 không chạy, không tạo series/authorization | Evidence: `reports/controlled-demo/20260729-crag-window-04-9b6f3ec/evaluation-window-declaration.json`, `window-outcome.json`, `pair-01/run.json` |

...
| Provider smoke đầu/cuối | Codex chạy lệnh; provider phải sẵn sàng | Đã phục hồi ngày 2026-07-18: smoke main-collection đầu/cuối đều 5/5, 0 retry, không có HTTP 503 và cùng provider-config hash; ba artifact 0/5 trước đó chỉ còn là lịch sử outage | Evidence hiện tại: `reports/controlled-demo/20260718-crag-controlled-demo-readiness/main-collection/provider-smoke.json` và `provider-smoke-final.json` |
| Ba staging baseline/candidate pairs | Codex chạy lệnh; SQL/Qdrant/provider phải sẵn sàng | Đã đạt với router `offline`: cả ba pair cùng commit/snapshot/manifest/config/concurrency, từng gate đều đạt; series guardrail `passed=true`, `production_eligible=true` | Evidence: `reports/controlled-demo/20260718-crag-controlled-demo-readiness/staging-offline-pair-01..03` và `crag-offline-series-guardrail.json` |
| Main-collection evaluation và CRAG high-risk review pack | Codex tạo pack; con người điền review | Đã chạy read-only trên `TaiLieuKyThuat_v2` và tạo pack 6 case. Candidate đạt 5/18 so với baseline 4/18; main gate chưa đạt, manifest mới có 18/20 case và chưa có human review | Giữ kết luận `inconclusive`; không dùng main-collection gate này thay fixture correction/repair chuyên dụng và không tạo case giả |

...
| Provider smoke cho từng pair | Codex chạy lệnh; provider phải sẵn sàng | Window 03 pair 01 đạt 5/5, 0 retry và provider không blocked. Pair 02/03 không chạy do pair 01 fail | Evidence: `reports/controlled-demo/20260729-crag-window-03-7d8cfa2/pair-01/provider-smoke.json` |
| Ba staging baseline/candidate pairs | Codex chạy lệnh; SQL/Qdrant/provider phải sẵn sàng | Metadata fast-path diagnostic đạt 5/5, correction P95 giảm từ `2110 ms` xuống `249 ms`. Window `20260729-crag-window-03-7d8cfa2` predeclare order baseline-first/candidate-first/baseline-first. Pair 01 fail: candidate đạt 5/9, có bốn generation error và hai Voyage rerank error; latency/cost/leakage/budget vẫn đạt nhưng `candidate_cases_passed`, `required_repairs_exercised`, `wrong_refusal_reduced` không đạt. Pair 02/03 không chạy, không tạo series/authorization | Evidence: `reports/controlled-demo/20260729-crag-window-03-7d8cfa2/evaluation-window-declaration.json`, `window-outcome.json`, `pair-01/run.json` |

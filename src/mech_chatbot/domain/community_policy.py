"""Domain-neutral policy for governed graph community summaries."""
...
def _approved_graph(edges) -> tuple[dict[str, set[str]], list[tuple]]:
    ...
    return adjacency, edge_ids_by_nodes


def _connected_components(adjacency: dict, edge_ids_by_nodes: list) -> list:
    components = []
    ...
    while unvisited:
        ...
        edge_ids = sorted(
            edge_id
            for edge_id, source, target in edge_ids_by_nodes
...
def detect_communities(
    ...
    *,
    ...
    adjacency, edge_ids_by_nodes = _approved_graph(edges)
    components = _connected_components(adjacency, edge_ids_by_nodes)
    return {
...
def _validate_sources(sources) -> list[dict]:
    ...
    for raw in sources or ():
        ...
        source["node_keys"] = sorted({
            str(value).strip()
            for value in source["node_keys"]
            if str(value).strip()
        }, key=str.casefold)
        try:
            source["edge_ids"] = sorted({
                int(value) for value in source["edge_ids"]
            })
        except (TypeError, ValueError) as exc:
            raise ValueError("complete source provenance is required") from exc
...
def _summary_provenance(node_keys, edge_ids, sources) -> tuple:
    nodes = sorted({
        str(value).strip()
        for value in node_keys or ()
        if str(value).strip()
    })
    ...
    mapped_nodes = {
        value for source in source_provenance for value in source["node_keys"]
    }
    mapped_edges = {
        value for source in source_provenance for value in source["edge_ids"]
    }
    ...
    return nodes, edges, source_provenance
...
def build_pending_summary(
    *,
    ...
    edge_ids,
    sources,
    generated_by="community-generator",
) -> dict:
    ...
    nodes, edges, source_provenance = _summary_provenance(
        node_keys, edge_ids, sources,
    )
...
def _evaluate_version(
    ...
    if (
        str(version.get("serving_epoch") or "")
        != str(summary.get("serving_epoch") or "")
        ...
        return SummaryServingDecision(
            False, "community_version_identity_mismatch",
        )
...
def _evaluate_runtime_identity(
    summary: dict,
    *,
    serving_epoch: str,
    graph_fingerprint: str,
...
def _validated_provenance(
    summary: dict,
) -> tuple[list[dict] | None, SummaryServingDecision | None]:
    try:
        provenance = _validate_sources(summary.get("source_provenance"))
    except ValueError:
        return None, SummaryServingDecision(
            False, "source_provenance_invalid",
        )
    ...
    mapped_nodes = {
        value for source in provenance for value in source["node_keys"]
    }
    mapped_edges = {
        value for source in provenance for value in source["edge_ids"]
    }
    if mapped_nodes != summary_nodes or mapped_edges != summary_edges:
        return None, SummaryServingDecision(
            False, "source_provenance_mapping_invalid",
        )
    return provenance, None


def _index_current_sources(current_sources) -> dict:
    current = {}
...
def _index_current_edges(current_edges) -> dict:
    edges = {}
...
def _evaluate_source_edge(
    ...
    for edge_id in source["edge_ids"]:
        ...
        if (
            ...
            return SummaryServingDecision(
                False, "source_edge_provenance_drift",
            )
    return None


def _access_scope(access_context: dict) -> tuple[bool, set[str], set[str], int]:
    context = access_context or {}
    roles = {
        str(value).strip().lower() for value in context.get("roles") or ()
    }
    departments = {
        str(value).strip()
        for value in context.get("allowed_departments") or ()
    }
    sites = {
        str(value).strip() for value in context.get("allowed_sites") or ()
    }
    max_level = _LEVELS.get(
        str(context.get("max_security_level") or "public").lower(), 0,
    )
    return "admin" in roles, departments, sites, max_level


def _evaluate_source_record(
    source: dict,
    current: dict,
    access_scope: tuple[bool, set[str], set[str], int],
) -> SummaryServingDecision | None:
    is_admin, departments, sites, max_level = access_scope
...
def evaluate_summary_serving(
    ...
    *,
    ...
    decision = _evaluate_version(summary, community_version or {})
    if decision:
        return decision
    decision = _evaluate_runtime_identity(
        summary,
        serving_epoch=serving_epoch,
        graph_fingerprint=graph_fingerprint,
    )
    if decision:
        return decision
    provenance, decision = _validated_provenance(summary)
    if decision:
        return decision

    edges = _index_current_edges(current_edges)
    for source in provenance or ():
        decision = _evaluate_source_edge(source, edges)
        if decision:
            return decision
    current = _index_current_sources(current_sources)
    scope = _access_scope(access_context)
    for source in provenance:
        decision = _evaluate_source_record(source, current, scope)
        if decision:
            return decision

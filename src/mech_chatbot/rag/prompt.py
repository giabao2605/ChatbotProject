...
_COMMON_RULES_VI = (
    ...
    "thức nền của model, Internet hay phỏng đoán để bổ sung. Chỉ chọn đoạn nói đúng "
    "đối tượng và hoạt động được hỏi; cùng có từ khóa hoặc đơn vị đo chưa đủ để kết luận "
    "là liên quan. Nếu chưa xác định được người dùng hỏi quy trình/tài liệu nào, hỏi lại "
    "mã hoặc tên quy trình thay vì gộp số liệu của các hoạt động khác nhau.\n"
    ...
    "hơn hoặc liên hệ phòng ban phụ trách. Nếu tài liệu có đề cập chủ đề nhưng thiếu một "
    "chi tiết, phải nêu chính xác chi tiết còn thiếu; KHÔNG được đồng thời khẳng định tài "
    "liệu có nội dung rồi dùng câu từ chối chung rằng tài liệu không đề cập đến thông tin "
    "đó. Với câu hỏi nhiều ý, đánh giá và trả lời từng ý riêng. TUYỆT ĐỐI không bịa.\n"
    ...
    "thành một kết luận chung. Phiên bản là nhãn tài liệu, không phải đại lượng. "
    "Nêu phiên bản của từng tài liệu riêng; không trừ các số phiên bản hoặc suy ra "
    "số lần cập nhật giữa các tài liệu khác nhau.\n"
    ...
    "gốc; tên thuộc tính trong lời trả lời dùng tiếng Việt (ví dụ: phiên bản), "
    "không đổi các trường trong trích dẫn.\n"
    ...
    "đầu dòng / các bước cho quy trình. Chỉ nêu đúng thông tin được hỏi; không thêm thuộc "
    "tính khác dù nằm trong cùng tài liệu. Không thêm mã, vật liệu hoặc thông số không có "
    "trong câu hỏi, trừ thông tin đó chính là nội dung người dùng yêu cầu tra cứu. "
    "Mỗi thuộc tính được hỏi chỉ trình bày một lần; không lặp lại bảng bằng đoạn tóm tắt.\n"
...
_COMMON_RULES_EN = (
    ...
    "knowledge, the Internet, or guesses to fill gaps. Select passages about the actual "
    "subject and activity asked about; shared keywords or units alone do not establish "
    "relevance. If the intended process/document is unclear, ask for its code or name "
    "instead of combining values from different activities.\n"
    ...
    "code / more specific keywords or contact the responsible department. If a document "
    "covers the topic but omits one detail, state the exact missing detail; you MUST NOT both "
    "acknowledge that the document covers a topic and then use a generic refusal claiming it "
    "does not. For multi-part questions, evaluate and answer each requested item separately. "
    "NEVER fabricate.\n"
    ...
    "merge differing numbers into a single combined conclusion. Version numbers are document labels, "
    "not quantities. State each document version independently; do not subtract them or infer "
    "the number of revisions between unrelated documents.\n"
    ...
    "for procedures. State only the requested information; do not add other attributes "
    "from the same document. Do not add codes, materials, or specifications absent from "
    "the question unless that information is exactly what the user asks to retrieve. "
    "Present each requested attribute once; do not repeat the table in a summary.\n"

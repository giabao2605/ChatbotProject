...
_GLOBAL_QUERY_TERMS = (
    "tổng hợp", "tổng quan", "toàn bộ tài liệu", "giữa các tài liệu",
    "xuyên tài liệu", "xu hướng chung", "bức tranh chung", "across documents",
    "across all documents", "overall summary", "global overview",
    "summarize all", "cross-document",
)
...
def is_global_query(question: str) -> bool:
    ...
    return bool(normalized) and any(term in normalized for term in _GLOBAL_QUERY_TERMS)
...
def load_community_context(
    ...
    *,
    ...
    if not community_enabled:
        return CommunityContextResult((), False, "community_summaries_disabled")
    if not is_global_query(question):
        return CommunityContextResult((), False, "query_not_global")
    if not str(serving_epoch or "").strip() or not str(graph_fingerprint or "").strip():
    ...
    try:
        ...
        return CommunityContextResult((), False, "community_repository_unavailable")
    if not summaries:
        return CommunityContextResult((), False, "no_approved_summary")
    documents = []
    used_summaries = 0
    for summary in summaries:
        hydrated = _hydrate_summary_documents(summary, client, collection_name)
        if not hydrated:
            continue
        used_summaries += 1
        ...
        if len(documents) >= max(1, int(max_documents)):
            ...
            break
    if not documents:
        return CommunityContextResult((), False, "summary_sources_not_hydrated")
...
__all__ = [
    "CommunityContextResult", "SummaryServingDecision", "build_pending_summary",
    "detect_communities", "enabled", "evaluate_summary_serving",
    "is_global_query", "load_community_context",
]

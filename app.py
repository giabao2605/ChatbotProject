...
if submission := st.chat_input("Nhap cau hoi ky thuat can tra cuu...", accept_file=True, file_type=UPLOAD_FILE_TYPES):
    ...
    if is_learning_batch:
        ...
        with st.chat_message("assistant"):
            ...
            fail_count = 0
            responses = []
            status_placeholder = st.status(f"Dang boc tach va nap {len(uploaded_files)} tai lieu...", expanded=True)
            with status_placeholder:
                ...
                for idx, uf in enumerate(uploaded_files):
                    ...
                    if file_size_mb > 100:
                        st.warning(f"File {uf.name} lon ({file_size_mb:.1f}MB) — qua trinh xu ly co the mat vai phut.")
 
                    success, msg_response = learn_new_file(file_path, safe_original_name, "Tu_Hoc", progress_callback=ui_progress_callback)
                    if success:
                        success_count += 1
                        responses.append(f"✅ **{uf.name}**: Nap thanh cong")
                    else:
                        fail_count += 1
                        responses.append(f"❌ **{uf.name}**: That bai ({msg_response})")
 
            final_status = f"Hoan tat nap du lieu! (Thanh cong {success_count}/{len(uploaded_files)})"

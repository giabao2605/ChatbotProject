...
def test_live_health_fetch_rejects_non_loopback_urls_before_network_call():
    with pytest.raises(campaign.CampaignStopped, match="non_loopback_runtime_url"):
        traffic.fetch_live_health(
            {"rag_url": "https://example.com", "control_url": "http://127.0.0.1:8210"},
...
def test_main_run_due_loads_service_token_from_settings_not_cli(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        traffic,
        "_parse_args",
        lambda: SimpleNamespace(command="run-due", root=tmp_path, dotenv=tmp_path / ".env"),
    )
